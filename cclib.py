...
class CcApi:
    ...
    # coincheck のプライベートAPIにリクエストを送信する関数
    def _private_api(self, i_path, i_nonce, i_params=None, i_method="get"):
        headers = {'ACCESS-KEY': self.API_KEY, 'ACCESS-NONCE': str(i_nonce), 'Content-Type': 'application/json'}
    ...
    def orderbook(self):
        ...
        j = c.json()
        return {
            "asks": [(float(i[0]), float(i[1])) for i in j["asks"]],
    ...
    def sell(self, price, amount):
        j = self._trace_api(price, amount, "sell")
        return j["id"]

    # 買い注文を実行する関数
    def buy(self, price, amount):
        j = self._trace_api(price, amount, "buy")
        return j["id"]
